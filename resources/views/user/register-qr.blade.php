...
@section('content')
    ...
    <div class="min-h-screen py-8 px-5 max-w-md mx-auto" style="background-color:#F0F0FA;">
        ...
        <div class="flex items-center mb-8">
            <div>
                <p class="f-display"
                    style="font-size:10px;font-weight:600;color:#9B9BB4;letter-spacing:.15em;text-transform:uppercase;margin-bottom:3px">
                    QwickReach</p>
                <h1 class="f-display" style="font-size:22px;font-weight:800;color:#1A1A3E;line-height:1.2">Register Your Tag
                </h1>
            </div>
        </div>
        ...
        {{-- ── QR INFO PILL ── --}}
        <div
            style="background:#fff;border-radius:20px;border:1px solid rgba(107,71,214,.15);padding:14px 18px;display:flex;align-items:center;gap:14px;margin-bottom:24px;box-shadow:0 10px 25px -5px rgba(107,71,214,0.1)">
            <div
                style="width:44px;height:44px;background:#F0F0FA;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;border:1px solid rgba(107,71,214,.2);">
                🏷️</div>
            <div style="flex:1;min-width:0">
                <p class="f-display"
                    style="font-size:9px;color:#9B9BB4;font-weight:700;text-transform:uppercase;letter-spacing:.12em;margin-bottom:2px">
                    Category</p>
                <p class="f-display" style="font-size:14px;font-weight:800;color:#1A1A3E">{{ $qrCode->category->name }}</p>
            </div>
            <div style="text-align:right;flex-shrink:0">
                <p class="f-display"
                    ...
                    QR Code</p>
                <p class="f-display"
                    style="font-size:12px;font-weight:800;color:#1A1A3E;background:#F0F0FA;padding:4px 10px;border-radius:8px;border:1px solid rgba(107,71,214,.2)">
                    {{ $qrCode->qr_code }}</p>
            </div>
        </div>
        ...
        <div
            ...
            <form action="{{ route('qr.store-qr-registration', $qrCode->id) }}" method="POST"
                ...
                <p class="f-display"
                    style="font-size:11px;font-weight:700;color:#9B9BB4;text-transform:uppercase;letter-spacing:.12em;margin-bottom:14px">
                    Personal Info</p>
                ...
                <div style="margin-bottom:20px;position:relative">
                    ...
                    <input type="tel" name="friend_family_2" value="{{ old('friend_family_2') }}"
                        ...
                        style="width:100%;padding:14px 60px 14px 44px;background:#F0F0FA;border:1.5px solid transparent;border-radius:16px;font-size:14px;font-weight:500;color:#1A1A3E;outline:none;transition:all .2s ease"
                        onfocus="this.style.border='1.5px solid #6B47D6';this.style.background='#fff'"
                        onblur="this.style.border='1.5px solid transparent';this.style.background='#F0F0FA'">
                    <span class="f-display"
                        style="position:absolute;right:14px;top:50%;transform:translateY(-50%);font-size:9px;font-weight:700;color:#A3A3C2;text-transform:uppercase">Optional</span>
                </div>
                ...
                <p class="f-display"
                    ...
                    Address</p>
                <div style="margin-bottom:20px;position:relative">
                    <div style="position:absolute;left:16px;top:16px;color:#A3A3C2">
                        <svg width="18" height="18" fill="currentColor" viewBox="0 0 20 20">
                            <path
                    ...
                    <textarea name="full_address" rows="3" placeholder="Full Address"
                        style="width:100%;padding:14px 16px 14px 44px;background:#F0F0FA;border:1.5px solid transparent;border-radius:16px;font-size:14px;font-weight:500;color:#1A1A3E;outline:none;transition:all .2s ease;resize:none"
                        onfocus="this.style.border='1.5px solid #6B47D6';this.style.background='#fff'"
                        onblur="this.style.border='1.5px solid transparent';this.style.background='#F0F0FA'">{{ old('full_address') }}</textarea>
                </div>
                    ...
                    @if ($categoryTag === 'pet')
                        <div style="margin-bottom:12px;">
                            <input type="file" name="photo" accept="image/*"
                                style="width:100%;padding:10px 16px;background:#F0F0FA;border:1.5px solid transparent;border-radius:16px;font-size:14px;font-weight:500;color:#1A1A3E;outline:none;transition:all .2s ease"
                                onfocus="this.style.border='1.5px solid #6B47D6';this.style.background='#fff'"
                                onblur="this.style.border='1.5px solid transparent';this.style.background='#F0F0FA'">
                        </div>
                        <div style="display:flex;gap:12px;margin-bottom:12px;">
                            <input type="text" name="breed" value="{{ old('breed') }}" placeholder="Breed"
                                style="flex:1;padding:14px 16px;background:#F0F0FA;border:1.5px solid transparent;border-radius:16px;font-size:14px;font-weight:500;color:#1A1A3E;outline:none;transition:all .2s ease"
                                onfocus="this.style.border='1.5px solid #6B47D6';this.style.background='#fff'"
                                onblur="this.style.border='1.5px solid transparent';this.style.background='#F0F0FA'">
                        ...
                        <div style="margin-bottom:20px;">
                            <input type="text" name="colour" value="{{ old('colour') }}" placeholder="Colour"
                                style="width:100%;padding:14px 16px;background:#F0F0FA;border:1.5px solid transparent;border-radius:16px;font-size:14px;font-weight:500;color:#1A1A3E;outline:none;transition:all .2s ease"
                                onfocus="this.style.border='1.5px solid #6B47D6';this.style.background='#fff'"
                                onblur="this.style.border='1.5px solid transparent';this.style.background='#F0F0FA'">
                        </div>
                    @elseif($categoryTag === 'car' || $categoryTag === 'bike')
                        ...
                        <div style="margin-bottom:20px;">
                            <input type="text" name="vehicle_no" required value="{{ old('vehicle_no') }}"
                                ...
                                style="width:100%;padding:14px 16px;background:#F0F0FA;border:1.5px solid transparent;border-radius:16px;font-size:14px;font-weight:500;color:#1A1A3E;outline:none;transition:all .2s ease;text-transform:uppercase;"
                                onfocus="this.style.border='1.5px solid #6B47D6';this.style.background='#fff'"
                                onblur="this.style.border='1.5px solid transparent';this.style.background='#F0F0FA'">
                        </div>
                ...
                <p class="f-display"
                    style="font-size:11px;font-weight:700;color:#9B9BB4;text-transform:uppercase;letter-spacing:.12em;margin-bottom:14px">
                    Category <span style="color:#6B47D6">· Auto-selected</span></p>
                ...
                <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:28px; pointer-events: none;">
                    ...
                    @foreach ($tags as $t)
                        ...
                        <label
                            ...
                            {{ $isActive ? 'background:#1A1A3E;border-color:#1A1A3E;' : 'background:#F0F0FA;border-color:transparent;opacity:.45;' }}">

                            <input type="checkbox" name="selected_tags[]" value="{{ $t['v'] }}"
                                {{ $isActive ? 'checked' : '' }} style="width:18px;height:18px;accent-color:#6B47D6;">
                            ...
                            @if ($isActive)
                                <span class="f-display"
                                    style="background:#6B47D6;color:#fff;font-size:9px;font-weight:800;padding:4px 10px;border-radius:100px;text-transform:uppercase">Selected</span>
                            @endif
                        </label>
                    @endforeach
                </div>
                ...
                <button type="submit"
                    style="width:100%;padding:17px;background:#1A1A3E;color:#fff;border:none;border-radius:20px;font-family:'Syne',sans-serif;font-size:15px;font-weight:800;cursor:pointer;box-shadow:0 12px 25px -5px rgba(26,26,62,0.3);transition:all .25s ease; border-bottom: 4px solid #6B47D6;"
        ...
        <div style="display:flex;align-items:center;gap:14px;justify-content:center;padding:8px 0 4px">
            <div style="height:1px;flex:1;background:linear-gradient(to right,transparent,rgba(107,71,214,.15))"></div>
            <p class="f-display" style="font-size:11px;font-weight:800;letter-spacing:.04em;color:#9B9BB4">Qwick<span
                    style="color:#6B47D6">Reach</span></p>
