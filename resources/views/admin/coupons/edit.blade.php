...
<div class="main-area">
    <div class="page-scroll" style="background: var(--bg); padding: 20px !important;">
        ...
        <div class="card" style="padding: 25px; border: 1px solid var(--border); display: flex; flex-direction: column; max-width: 1200px; background: var(--card); border-radius: var(--radius);">
            <form action="{{ route('admin.coupons.update', $coupon->id) }}" method="POST" style="display: flex; flex-direction: column; gap: 20px;">
                @csrf
                @method('PUT')
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
                    ...
                    <div>
                        ...
                        <select id="user_id" name="user_id" required style="width: 100%; background: var(--card2); border: 1px solid var(--border); color: var(--text); border-radius: 10px; padding: 12px 15px; outline: none; font-size: 13px;">
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" data-name="{{ $user->name }}" {{ old('user_id', $coupon->user_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div style="display: flex;">
                            <input type="text" name="code" id="code" value="{{ old('code', $coupon->code) }}" required style="flex: 1; background: var(--card2); border: 1px solid var(--border); color: var(--blue); border-right: none; border-radius: 10px 0 0 10px; padding: 12px 15px; outline: none; font-size: 13px; font-weight: 800; letter-spacing: 1px;" placeholder="e.g. USER2023X">
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_amount" id="discount_amount" step="0.01" min="0" value="{{ old('discount_amount', rtrim(rtrim($coupon->discount_amount, '0'), '.')) }}" required style="width: 100%; background: var(--card2); border: 1px solid var(--border); color: var(--text); border-radius: 10px; padding: 12px 15px; outline: none; font-size: 13px;" placeholder="Enter amount or percentage">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="expires_at" id="expires_at" value="{{ old('expires_at', $coupon->expires_at ? $coupon->expires_at->format('Y-m-d') : '') }}" style="width: 100%; background: var(--card2); border: 1px solid var(--border); color: var(--text); border-radius: 10px; padding: 12px 15px; outline: none; font-size: 13px;">
                        <p style="margin: 4px 0 0 0; color: var(--text3); font-size: 10px;">Leave blank for lifetime validity.</p>
                    </div>

                    <!-- Status -->
                    <div style="display: flex; align-items: center; margin-top: 25px;">
                        <label class="toggle-pill {{ old('is_active', $coupon->is_active) ? 'on' : '' }}" id="status_toggle">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $coupon->is_active) ? 'checked' : '' }} style="display: none;">
                            <div class="knob"></div>
                        </label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle form submit to ensure checkbox sends proper value if checked. If not checked it's excluded from POST by default HTML behavior.
});
</script>
@endsection
